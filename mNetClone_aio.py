...
class mNET:
    ...
    def __init__(self, db, net_id, cfg, write_flag):
        ...
        if not self.WRITE: print(f'Module WRITE IS FALSE!!!')
        self.net_id = net_id
    ...
    async def sync(self):
        ...
        #await self.u_getNetworkSnmp()
        #await self.u_getNetworkTrafficAnalysis()
        try: await self.u_getNetworkSyslogServers()
        except:
    ...
    async def u_getNetwork(self):
        self.getNetwork = await self.db.networks.getNetwork(self.net_id)
        return self.getNetwork
    
    async def u_getNetworkAlertsSettings(self):
        self.getNetworkAlertsSettings = await self.db.networks.getNetworkAlertsSettings(self.net_id)
        return self.getNetworkAlertsSettings

    async def u_getNetworkGroupPolicies(self):
        self.getNetworkGroupPolicies = await self.db.networks.getNetworkGroupPolicies(self.net_id)
        return self.getNetworkGroupPolicies
    
    async def u_getNetworkSnmp(self):
        self.getNetworkSnmp = await self.db.networks.getNetworkSnmp(self.net_id)
        return self.getNetworkSnmp

    async def u_getNetworkTrafficAnalysis(self):
        self.getNetworkTrafficAnalysis = await self.db.networks.getNetworkTrafficAnalysis(self.net_id)
        return self.getNetworkTrafficAnalysis

    async def u_getNetworkSyslogServers(self):
        self.getNetworkSyslogServers = await self.db.networks.getNetworkSyslogServers(self.net_id)
        return self.getNetworkSyslogServers
        
    async def u_getNetworkWebhooksHttpServers(self):
        self.getNetworkWebhooksHttpServers = await self.db.networks.getNetworkWebhooksHttpServers(self.net_id)
        return self.getNetworkWebhooksHttpServers
        
       
    async def u_getNetworkWirelessSettings(self):
        self.getNetworkWirelessSettings = await self.db.wireless.getNetworkWirelessSettings(self.net_id)
        return self.getNetworkWirelessSettings
        
    async def u_getNetworkWirelessBluetoothSettings(self):
        self.getNetworkWirelessBluetoothSettings = await self.db.wireless.getNetworkWirelessBluetoothSettings(self.net_id)
        return self.getNetworkWirelessBluetoothSettings
        
    async def u_getNetworkWirelessRfProfiles(self):
        self.getNetworkWirelessRfProfiles = await self.db.wireless.getNetworkWirelessRfProfiles(self.net_id)
    ...
    async def u_getSSIDS_l3(self):
        self.ssids_l3 = []
        for ssid_num in range(0,15):
            if ssid_num in self.ssids_range:
                self.ssids_l3.append(await self.db.wireless.getNetworkWirelessSsidFirewallL3FirewallRules(self.net_id, ssid_num))
            else:
                self.ssids_l3.append([])
        return self.ssids_l3

    async def u_getSSIDS_l7(self):
        self.ssids_l7 = []
        for ssid_num in range(0,15):
            if ssid_num in self.ssids_range:
                self.ssids_l7.append(await self.db.wireless.getNetworkWirelessSsidFirewallL7FirewallRules(self.net_id, ssid_num))
            else:
                self.ssids_l7.append([])
        return self.ssids_l7

    async def u_getSSIDS_ts(self):
        self.ssids_ts = []
        for ssid_num in range(0,15):
            if ssid_num in self.ssids_range:
                self.ssids_ts.append(await self.db.wireless.getNetworkWirelessSsidTrafficShapingRules(self.net_id, ssid_num))
            else:
                self.ssids_ts.append([])
        return self.ssids_ts

    async def u_getSSIDS_ipsk(self):    
        self.ssids_ipsk = []
        for ssid_num in range(0,15):
            if ssid_num in self.ssids_range:
                self.ssids_ipsk.append(await self.db.wireless.getNetworkWirelessSsidIdentityPsks(self.net_id, ssid_num))
            else:
                self.ssids_ipsk.append([])
        return self.ssids_ipsk
 

    async def u_getNetworkSwitchMtu(self):
        self.getNetworkSwitchMtu = await self.db.switch.getNetworkSwitchMtu(self.net_id)
        return self.getNetworkSwitchMtu
        
    async def u_getNetworkSwitchSettings(self):
        self.getNetworkSwitchSettings = await self.db.switch.getNetworkSwitchSettings(self.net_id)
        return self.getNetworkSwitchSettings
        
    async def u_getNetworkSwitchDscpToCosMappings(self):
        self.getNetworkSwitchDscpToCosMappings = await self.db.switch.getNetworkSwitchDscpToCosMappings(self.net_id)
        return self.getNetworkSwitchDscpToCosMappings
        
    async def u_getNetworkSwitchRoutingMulticast(self):
        self.getNetworkSwitchRoutingMulticast = await self.db.switch.getNetworkSwitchRoutingMulticast(self.net_id)
        return self.getNetworkSwitchRoutingMulticast
       
    async def u_getNetworkSwitchAccessControlLists(self):
        self.getNetworkSwitchAccessControlLists = await self.db.switch.getNetworkSwitchAccessControlLists(self.net_id)
        return self.getNetworkSwitchAccessControlLists
        
    async def u_getNetworkSwitchStormControl(self):
        try:
            self.getNetworkSwitchStormControl = await self.db.switch.getNetworkSwitchStormControl(self.net_id)
        except:
    ...
    async def u_getNetworkSwitchQosRules(self):
        self.getNetworkSwitchQosRules = await self.db.switch.getNetworkSwitchQosRules(self.net_id)
        return self.getNetworkSwitchQosRules

    async def u_getNetworkSwitchQosRulesOrder(self):
        self.getNetworkSwitchQosRulesOrder = await self.db.switch.getNetworkSwitchQosRulesOrder(self.net_id)
        return self.getNetworkSwitchQosRulesOrder
    
    ##MG update functions
    async def u_getNetworkCellularGatewayDhcp(self):
        self.getNetworkCellularGatewayDhcp = await self.db.cellularGateway.getNetworkCellularGatewayDhcp(self.net_id)
        return self.getNetworkCellularGatewayDhcp

    async def u_getNetworkCellularGatewaySubnetPool(self):
        self.getNetworkCellularGatewaySubnetPool = await self.db.cellularGateway.getNetworkCellularGatewaySubnetPool(self.net_id)
        return self.getNetworkCellularGatewaySubnetPool

    async def u_getNetworkCellularGatewayUplink(self):
        self.getNetworkCellularGatewayUplink = await self.db.cellularGateway.getNetworkCellularGatewayUplink(self.net_id)
        return self.getNetworkCellularGatewayUplink

    async def u_getNetworkCellularGatewayConnectivityMonitoringDestinations(self):
        self.getNetworkCellularGatewayConnectivityMonitoringDestinations = await self.db.cellularGateway.getNetworkCellularGatewayConnectivityMonitoringDestinations(self.net_id)
    ...
    async def cloneFrom(self, master):
        if not self.CLEAN:
    ...
    async def NET_cloneFrom(self, master):
        ...
        if not self.compare(master.getNetworkTrafficAnalysis, self.getNetworkTrafficAnalysis):
            if self.WRITE:
                ...
                self.CLEAN = False
                await self.db.networks.updateNetworkTrafficAnalysis(self.net_id,**master.getNetworkTrafficAnalysis)
        if not self.CLEAN:
            await self.u_getNetworkTrafficAnalysis()
            self.CLEAN = True
        ...
        if not self.soft_compare(master.getNetworkSyslogServers['servers'], self.getNetworkSyslogServers['servers']): 
            ...
            if self.WRITE: 
                ...
                self.CLEAN = False
                newSyslogServers = copy.deepcopy(master.getNetworkSyslogServers)
                for s in newSyslogServers['servers']:
                    if not 'wireless' in self.getNetwork['productTypes']:
                        if 'Air Marshal events' in s['roles']:s['roles'].remove('Air Marshal events')
                        if 'Wireless event log' in s['roles']:s['roles'].remove('Wireless event log')
                    if not 'switch' in self.getNetwork['productTypes']:
                        if 'Switch event log' in s['roles']: s['roles'].remove('Switch event log')
                    if not 'appliance' in self.getNetwork['productTypes'] and not 'wireless' in self.getNetwork['productTypes']:
                        if 'URLs' in s['roles']: s['roles'].remove('URLs')
                        if 'Flows' in s['roles']: s['roles'].remove('Flows')
                    if not 'appliance' in self.getNetwork['productTypes'] and not 'cellularGateway' in self.getNetwork['productTypes']:
                        if 'Appliance event log' in s['roles']:s['roles'].remove('Appliance event log')
                        if 'Security events' in s['roles']:s['roles'].remove('Security events')
                    if not 'cellularGateway' in self.getNetwork['productTypes']: #MG use MX alerting/eventlog. doesn't have security tho
                        if 'Security events' in s['roles']:s['roles'].remove('Security events')

                try:
                    await self.db.networks.updateNetworkSyslogServers(self.net_id, **{'servers': []})
                    #await self.db.networks.updateNetworkSyslogServers(self.net_id,**master.getNetworkSyslogServers)
                    await self.db.networks.updateNetworkSyslogServers(self.net_id,**newSyslogServers) #this should have syslog filtered for devices
                except:
        ...
        if not self.CLEAN:
            await self.u_getNetworkSyslogServers()
            self.CLEAN = True
        ## / end-Syslog Settings
        ...
        if not self.compare(master.getNetworkSnmp, self.getNetworkSnmp):
            if self.WRITE: 
                ...
                self.CLEAN = False
                await self.db.networks.updateNetworkSnmp(self.net_id,**master.getNetworkSnmp)
        ## / end-SNMP Settings
        if not self.CLEAN:
            await self.u_getNetworkSnmp()
            self.CLEAN = True
        ...
        if not self.compare(master.getNetworkAlertsSettings, self.getNetworkAlertsSettings):
            if self.WRITE:
                self.CLEAN = False
                try:
                    await self.db.networks.updateNetworkAlertsSettings(self.net_id, **master.getNetworkAlertsSettings)
                except:
            ...
            if self.WRITE:
                ...
                if local_gp == None:
                    ...
                    if not self.soft_compare(tempGP, self.find_fromName(master.getNetworkGroupPolicies, tempGP['name'])):
                        ...
                        self.CLEAN = False
                        await self.db.networks.updateNetworkGroupPolicy(self.net_id, **tempGP)
                    #else:
    ...
    async def MS_cloneFrom(self, master):
        ...
        if not self.compare(master.getNetworkSwitchMtu, self.getNetworkSwitchMtu):
            if self.WRITE:
                ...
                await self.db.switch.updateNetworkSwitchMtu(self.net_id, **master.getNetworkSwitchMtu)
            if not self.CLEAN:
        ...
        if not self.compare(master.getNetworkSwitchSettings, self.getNetworkSwitchSettings):
            if self.WRITE:
                ...
                await self.db.switch.updateNetworkSwitchSettings(self.net_id, **master.getNetworkSwitchSettings)
            if not self.CLEAN:
        ...
        if not self.compare(master.getNetworkSwitchDscpToCosMappings, self.getNetworkSwitchDscpToCosMappings):
            if self.WRITE:
                ...
                await self.db.switch.updateNetworkSwitchDscpToCosMappings(self.net_id, **master.getNetworkSwitchDscpToCosMappings)
            if not self.CLEAN:
        ...
        if not self.compare(master.getNetworkSwitchRoutingMulticast, self.getNetworkSwitchRoutingMulticast):
            if self.WRITE:
                ...
                await self.db.switch.updateNetworkSwitchRoutingMulticast(self.net_id, **master.getNetworkSwitchRoutingMulticast)
            if not self.CLEAN:
        ...
        if not self.compare(master.getNetworkSwitchAccessControlLists, self.getNetworkSwitchAccessControlLists):
            if self.WRITE:
                ...
                await self.db.switch.updateNetworkSwitchAccessControlLists(self.net_id, **acls)
            if not self.CLEAN:
       ...
       #StormControl Settings
        if not self.compare(master.getNetworkSwitchStormControl, self.getNetworkSwitchStormControl):
            if self.WRITE:
                ...
                try:
                    await self.db.switch.updateNetworkSwitchStormControl(self.net_id, **master.getNetworkSwitchStormControl)                
                except:
            ...
            for rid in rOrder_src:
                ...
                if rule == None:
                    ...
                    try:
                        ...
                        await self.db.switch.createNetworkSwitchQosRule(self.net_id,**rule)
    ...
    async def MG_cloneFrom(self, master):
        ...
        if not self.compare(master.getNetworkCellularGatewayDhcp, self.getNetworkCellularGatewayDhcp):
            if self.WRITE and not master.getNetworkCellularGatewayDhcp == None:
                ...
                self.CLEAN = False
                await self.db.cellularGateway.updateNetworkCellularGatewayDhcp(self.net_id, **master.getNetworkCellularGatewayDhcp)
            if not self.CLEAN:
        ...
        if not self.compare(masterSub, selfSub):
            if self.WRITE and not master.getNetworkCellularGatewaySubnetPool == None:
                ...
                self.CLEAN = False
                await self.db.cellularGateway.updateNetworkCellularGatewaySubnetPool(self.net_id, **master.getNetworkCellularGatewaySubnetPool)
            if not self.CLEAN:
        ...
        if not self.compare(master.getNetworkCellularGatewayUplink, self.getNetworkCellularGatewayUplink):
            if self.WRITE and not master.getNetworkCellularGatewayUplink == None:
                ...
                self.CLEAN = False
                await self.db.cellularGateway.updateNetworkCellularGatewayUplink(self.net_id, **master.getNetworkCellularGatewayUplink)
            if not self.CLEAN:
        ...
        if not self.compare(master.getNetworkCellularGatewayConnectivityMonitoringDestinations, self.getNetworkCellularGatewayConnectivityMonitoringDestinations):
            if self.WRITE and not master.getNetworkCellularGatewayConnectivityMonitoringDestinations == None:
                ...
                try:
                    ...
                    await self.db.cellularGateway.updateNetworkCellularGatewayConnectivityMonitoringDestinations(self.net_id, **master.getNetworkCellularGatewayConnectivityMonitoringDestinations)
                except:
                    #something
                    pass
    ...
    async def MR_cloneFrom(self, master):
        ...
        for i in range(0,15):
            ...
            if not self.compare(self.ssids_l3[i], master.ssids_l3[i]):
                ...
                if self.WRITE: 
                    self.ssids_l3[i] = await self.db.wireless.updateNetworkWirelessSsidFirewallL3FirewallRules(self.net_id,i, **newL3)
                    self.CLEAN = False
                    CLEAN_L3 = False
            ...
            if not self.compare(self.ssids_l7[i], master.ssids_l7[i]):
                ...
                if self.WRITE: 
                    self.ssids_l7[i] = await self.db.wireless.updateNetworkWirelessSsidFirewallL7FirewallRules(self.net_id,i, **l7rules)
                    self.CLEAN = False
                    CLEAN_L7 = False
            ...
            if not self.compare(self.ssids_ts[i], master.ssids_ts[i]):
                ...
                try:
                    ...
                    if self.WRITE: 
                        self.ssids_ts[i] = await self.db.wireless.updateNetworkWirelessSsidTrafficShapingRules(self.net_id, i, **TSrules)
                        self.CLEAN = False
                        CLEAN_TS = False
                except:
        ...
        for ssid_num in self.ssids_range:
            ...
            for m_ipsk in master.ssids_ipsk[ssid_num]:
                    ...
                    if new_ipsk['name'] == s_ipsk['name']:
                        ...
                        if not new_ipsk['passphrase'] == s_ipsk['passphrase']: #if passwords are different, delete the ipsk and re-create
                            if self.WRITE:
                                self.CLEAN = False
                                try:
                                    await self.db.wireless.deleteNetworkWirelessSsidIdentityPsk(self.net_id, ssid_num, s_ipsk['id'])
                                except:
                ...
                if not exists and self.WRITE:
                    self.CLEAN = False
                    try:
                        await self.db.wireless.createNetworkWirelessSsidIdentityPsk(self.net_id, ssid_num, **new_ipsk)
                    except:
        ...
        if not self.compare(master.getNetworkWirelessSettings, self.getNetworkWirelessSettings):
            if self.WRITE: 
                ...
                self.CLEAN = False
                await self.db.wireless.updateNetworkWirelessSettings(self.net_id,**master.getNetworkWirelessSettings)
        if not self.CLEAN:
        ...
        if not self.compare(master.getNetworkWirelessBluetoothSettings, self.getNetworkWirelessBluetoothSettings):
            if self.WRITE: 
                ...
                await self.db.wireless.updateNetworkWirelessBluetoothSettings(self.net_id,**btCFG)
        if not self.CLEAN:
    ...
    async def wipeALL(self):
        ...
        for rfp in current:
            if self.WRITE: await self.db.wireless.deleteNetworkWirelessRfProfile(self.net_id, rfp['id'])
        ...
        for i in self.ssids_range:
            ...
            for ipsk in temp_ipsks:
                if self.WRITE: 
                    #try:
                    await self.db.wireless.deleteNetworkWirelessSsidIdentityPsk(self.net_id,i,ipsk['id'])
                    #except:
        ...
        for i in self.ssids_range:
            ...
            if self.WRITE: await self.db.wireless.updateNetworkWirelessSsidFirewallL3FirewallRules(self.net_id,i, rules = [], allowLanAccess = True)
        ...
        for i in self.ssids_range:
            ...
            if len(temp_l7fw) == 0:
                continue 
            if self.WRITE: await self.db.wireless.updateNetworkWirelessSsidFirewallL7FirewallRules(self.net_id,i, rules = [])
        ...
        for i in self.ssids_range:
            ...
            if not self.compare(temp_ts, defaultTS):
                if self.WRITE: await self.db.wireless.updateNetworkWirelessSsidTrafficShapingRules(self.net_id,i, **defaultTS)
        ...
        if self.WRITE:
            ...
            await self.db.wireless.updateNetworkWirelessSettings(self.net_id, **tmp_ws)
        

        #Bluetooth
        if self.WRITE:
            await self.db.wireless.updateNetworkWirelessBluetoothSettings(self.net_id, **{'scanningEnabled': False, 'advertisingEnabled': False})

        #Group-Policies
        for t_gp in self.getNetworkGroupPolicies:
            if self.WRITE:
                print(f"Wiping GroupPolicyId['{t_gp['groupPolicyId']}']")
                await self.db.networks.deleteNetworkGroupPolicy(self.net_id, t_gp['groupPolicyId'])
        ...
        for wh in self.getNetworkWebhooksHttpServers:
            if self.WRITE:
                await self.db.networks.deleteNetworkWebhooksHttpServer(self.net_id,wh['id'])
        ...
        if self.WRITE:
            await self.db.networks.updateNetworkAlertsSettings(self.net_id, **cleared)
  

        #SNMP
        if self.WRITE:
            await self.db.networks.updateNetworkSnmp(self.net_id, **{'access' : 'none'})

        #Traffic Shaping
        if self.WRITE:
            await self.db.networks.updateNetworkTrafficAnalysis(self.net_id, **{'mode' : 'disabled'})
        ...
        #MTU
        if self.WRITE:
            await self.db.switch.updateNetworkSwitchMtu(self.net_id, **{'defaultMtuSize': 9578, 'overrides': []})
        
        #Default VLAN
        if self.WRITE:
            await self.db.switch.updateNetworkSwitchSettings(self.net_id, **{'vlan': 1, 'useCombinedPower': False, 'powerExceptions': []})
        ...
        if self.WRITE:
            await self.db.switch.updateNetworkSwitchDscpToCosMappings(self.net_id, **defaultDSCPCOS)


        #Multicast
        defaultMC = {'defaultSettings': {'igmpSnoopingEnabled': True, 'floodUnknownMulticastTrafficEnabled': True}, 'overrides': []}
        if self.WRITE:
            await self.db.switch.updateNetworkSwitchRoutingMulticast(self.net_id, **defaultMC)
        ...
        defaultACL = {'rules': []}
        if self.WRITE:
            await self.db.switch.updateNetworkSwitchAccessControlLists(self.net_id, **defaultACL)


        #QoS Rules
        if self.WRITE:
            for q_rid in self.getNetworkSwitchQosRules:
                await self.db.switch.deleteNetworkSwitchQosRule(self.net_id, q_rid['id'])
